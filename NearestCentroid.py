...
print("Load data from pickle batches...")
X_train_flat, y_train, X_test_flat, y_test = load_cifar10(CIFAR10_DIR)

print(f"Shape of των τrain data : {X_train_flat.shape}")
print(f"Shape of Test data : {X_test_flat.shape}")
...
class NearestCentroidFromScratch:
    ...
    def fit(self, X, y):
        ...
        print(f"Centroid Calculation  {len(self.classes)} κλάσεις.")
...
def main():
    ...
    try:
        X_train_flat, y_train, X_test_flat, y_test = load_cifar10(CIFAR10_DIR)
        print(f"Load Data: Train {X_train_flat.shape}, Test {X_test_flat.shape}")
    except FileNotFoundError:
        print(f"CIFAR-10 data  NOT found directory in path '{CIFAR10_DIR}'.")
